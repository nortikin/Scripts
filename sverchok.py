...
# 2013-09-03 some bugfixes, default max, errors in first start etc.
...
def cache_read(cache):
    ...
    return(True, recipient, donor, centres, idx_centre, formula, diap_min, diap_max)
    
    
# store information in the cache
def cache_write(cache, recipient, donor, centres, idx_centre, formula, diap_min, diap_max):
...
class SurfOutput(Node, RecepCustomTreeNode):
    ...
    def init(self, context):
        ...
        self.inputs.new('NodeSocketFloat', "Max","MaxScale").default_value = 1.0
...
def drawit(cache, total_x=0,total_y=0,total_z=0, redraw=False):
    ...
    rd_cache = cache_read(cache)
    #True, recipient, donor, centres, idx_centre, formula, diap_min, diap_max
    
    # get objects (достаём объекты)
    formula = rd_cache[5]
    recipient_name = rd_cache[1] # имя полигона реципиента
    recipient = bpy.data.objects[recipient_name] #bpy.data.objects['polygons']
    donor_name = rd_cache[2] #per_cache[cache]['donor']
    donor = bpy.data.objects[donor_name] #bpy.data.objects['donor']
    mw2 = recipient.matrix_world
    mw3 = donor.matrix_world
    mesh2 = recipient.data
    mesh2.name = recipient_name
    mesh2.update()
    mesh3 = donor.data
    mesh3.name = donor_name
    pol2 = mesh2.polygons
    edg2 = mesh2.edges
    ver2 = mesh2.vertices
    dist = []

    sc_pols = scale_pols(pol2, ver2, donor)
